...
Container::Container()
    : fd(-1), version(0), extensions(), iova_next(0), hasIommu(false),
      groups(), log(logging.get("kernel:vfio::Container"))
{
        ...
        for(const char *module : requiredKernelModules) {
                if(kernel::loadModule(module) != 0) {
                        throw RuntimeError(
                            "Kernel module '{}' required but could not be "
                            "loaded. "
...
void Container::attachGroup(std::shared_ptr<Group> group)
{
        ...
        int ret
            = ioctl(group->getFileDescriptor(), VFIO_GROUP_SET_CONTAINER, &fd);
        if(ret < 0)
                throw SystemError(
                    "Failed to attach VFIO group {} to container fd {}",
                    group->getIndex(),
                    fd);
...
std::shared_ptr<Device> Container::attachDevice(pci::Device &pdev)
{
        ...
        if(pdev.getDriver() != kernelDriver) {
                log->debug("Bind PCI card to kernel driver '{}'",
                           kernelDriver);
        ...
        try {
                ...
                throw RuntimeError(
                    "BAR of device is in inconsistent state. Rewriting the "
                    "BAR "
        ...
        log->info("Attach to device {} with index {}",
                  std::string(name),
                  index);
        auto group = getOrAttachGroup(index);
...
bool Container::memoryUnmap(uintptr_t phys, size_t length)
{
        ...
        log->debug("DMA unmap size={:#x}, iova={:#x}",
                   dmaUnmap.size,
                   dmaUnmap.iova);
