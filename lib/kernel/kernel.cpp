...
int villas::kernel::setModuleParam(const char *module,
                                   const char *param,
                                   const char *value)
{
        FILE *f;
        ...
        if(!f)
                throw RuntimeError(
                    "Failed set parameter {} for kernel module {} to {}",
                    module,
                    param,
                    value);

        auto logger = logging.get("kernel");
        logger->debug("Set parameter {} of kernel module {} to {}",
                      module,
                      param,
                      value);
...
int villas::kernel::getCmdlineParam(const char *param, char *buf, size_t len)
{
        ...
        do {
                ...
                if(ret >= 1) {
                        auto logger = logging.get("kernel");
                        if(ret >= 2)
                                logger->debug("Found kernel param: {}={}",
                                              key,
                                              value);
                        else
...
int villas::kernel::getNrHugepages()
{
        ...
        if(!f) {
                ...
                logger->error("Failed to open {}: {}",
                              PROCFS_PATH "/sys/vm/nr_hugepages",
                              strerror(errno));
                return -1;
        }
