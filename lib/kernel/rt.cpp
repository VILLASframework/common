...
void init(int priority, int affinity)
{
        ...
        if(!is_rt)
                logger->warn(
                    "We recommend to use an PREEMPT_RT patched kernel!");

        if(priority)
                setPriority(priority);
        else
                logger->warn(
                    "You might want to use the 'priority' setting to "
                    "increase " PROJECT_NAME "'s process priority");

        if(affinity) {
                is_isol
                    = getCmdlineParam("isolcpus", isolcpus, sizeof(isolcpus));
                if(is_isol)
                        logger->warn(
                            "You should reserve some cores for " PROJECT_NAME
                            " (see 'isolcpus')");
                else {
                        ...
                        if(cset_non_isol.count() > 0)
                                logger->warn(
                                    "Affinity setting includes cores which "
                                    "are not isolated: affinity={}, "
                                    "isolcpus={}, non_isolated={}",
                                    (std::string) cset_pin,
                                    (std::string) cset_isol,
                ...
                logger->warn(
                    "You might want to use the 'affinity' setting to "
                    "pin " PROJECT_NAME " to dedicate CPU cores");
#else
...
void setThreadAffinity(pthread_t thread, int affinity)
{
        ...
        logger->debug("Set affinity of thread {} to {} {}",
