...
DeviceList::DeviceList()
{
        ...
        while((e = readdir(dp))) {
                ...
                if(ret != 4)
                        throw RuntimeError(
                            "Failed to parse PCI slot number: {}",
                            e->d_name);
...
DeviceList::value_type DeviceList::lookupDevice(const Slot &s)
{
        return *std::find_if(
            ...
            [s](const DeviceList::value_type &d) { return d->slot == s; });
}
...
DeviceList::value_type DeviceList::lookupDevice(const Id &i)
{
        return *std::find_if(
            ...
            [i](const DeviceList::value_type &d) { return d->id == i; });
}
...
DeviceList::value_type DeviceList::lookupDevice(const Device &d)
{
        auto dev = std::find_if(
            ...
            [d](const DeviceList::value_type &e) { return *e == d; });
...
Id::Id(const std::string &str) : vendor(0), device(0), class_code(0)
{
        ...
        if(!s) {
                free(tmp);
                throw RuntimeError("Failed to parse PCI id: ':' expected",
                                   str);
        }
        ...
        *s++ = 0;
        if(tmp[0] && strcmp(tmp, "*")) {
                ...
                if((e && *e) || (x < 0 || x > 0xffff)) {
                        free(tmp);
                        throw RuntimeError(
                            "Failed to parse PCI id: {}: Invalid vendor id",
                            str);
                }
        ...
        if(s[0] && strcmp(s, "*")) {
                ...
                if((e && *e) || (x < 0 || x > 0xffff)) {
                        free(tmp);
                        throw RuntimeError(
                            "Failed to parse PCI id: {}: Invalid device id",
                            str);
                }
        ...
        if(c && c[0] && strcmp(s, "*")) {
                ...
                if((e && *e) || (x < 0 || x > 0xffff)) {
                        free(tmp);
                        throw RuntimeError(
                            "Failed to parse PCI id: {}: Invalid class code",
                            str);
                }
...
Slot::Slot(const std::string &str) : domain(0), bus(0), device(0), function(0)
{
                ...
                *colon++ = 0;
                ...
                if(buss[0] && strcmp(buss, "*")) {
                        ...
                        if((e && *e) || (x < 0 || x > 0xff)) {
                                free(tmp);
                                throw RuntimeError(
                                    "Failed to parse PCI slot: {}: invalid "
                                    "bus",
                                    str);
                        }
        ...
        if(mid[0] && strcmp(mid, "*")) {
                ...
                if((e && *e) || (x < 0 || x > 0x1f)) {
                        free(tmp);
                        throw RuntimeError(
                            "Failed to parse PCI slot: {}: invalid slot",
                            str);
                }
        ...
        if(dot && dot[0] && strcmp(dot, "*")) {
                ...
                if((e && *e) || (x < 0 || x > 7)) {
                        free(tmp);
                        throw RuntimeError(
                            "Failed to parse PCI slot: {}: invalid function",
                            str);
                }
...
std::list<Region> Device::getRegions() const
{
        ...
        snprintf(sysfs,
                 sizeof(sysfs),
                 "%s/bus/pci/devices/%04x:%02x:%02x.%x/resource",
                 SYSFS_PATH,
                 slot.domain,
                 slot.bus,
                 slot.device,
        ...
        while(reg_num < 8 && (bytesRead = getline(&line, &len, f)) != -1) {
                ...
                for(int i = 0; i < 3; i++) {
                        ...
                        if(s == end) {
                                log->debug("Error parsing line {} of {}",
                                           reg_num + 1,
                                           sysfs);
...
bool Device::attachDriver(const std::string &driver) const
{
        ...
        snprintf(fn,
                 sizeof(fn),
                 "%s/bus/pci/drivers/%s/new_id",
                 SYSFS_PATH,
                 driver.c_str());
        f = fopen(fn, "w");
        if(!f)
                throw SystemError("Failed to add PCI id to {} driver ({})",
                                  driver,
                                  fn);

        log->info("Adding ID to {} module: {:04x} {:04x}",
                  driver,
                  id.vendor,
                  id.device);
...
uint32_t Device::readHostBar(unsigned barNum) const
{
        ...
        if(std::sscanf(line.c_str(), "%llx %llx %llx", &start, &end, &flags)
           != 3)
        ...
        log->debug("Host BAR: start={:#x}, end={:#x}, size={:#x}, flags={:#x}",
                   start,
                   end,
                   end - start,
                   flags);
                               ...
                               std::ios_base::openmode mode) const
{
        ...
        auto sysFsFilename
            = fmt::format("{}/bus/pci/devices/{:04x}:{:02x}:{:02x}.{:x}/{}",
                          SYSFS_PATH,
                          slot.domain,
                          slot.bus,
                          slot.device,
