      ...
      log(logging.get("kernel:vfio::Device"))
{
        ...
        // Open device fd
        fd = ioctl(groupFileDescriptor,
                   VFIO_GROUP_GET_DEVICE_FD,
                   name.c_str());
        if(fd < 0)
                throw RuntimeError("Failed to open VFIO device: {}",
                                   name.c_str());
        ...
        if(ret < 0)
                throw RuntimeError("Failed to get VFIO device info for: {}",
                                   name);
        ...
        for(size_t i = 0; i < info.num_regions && i < 8; i++) {
                ...
                if(ret < 0)
                        throw RuntimeError(
                            "Failed to get region {} of VFIO device: {}",
                            i,
                            name);

                log->debug(
                    "region {} info: flags: 0x{:x}, cap_offset: 0x{:x}, size: "
                    "0x{:x}, offset: 0x{:x}",
                    region.index,
                    region.flags,
                    region.cap_offset,
        ...
        for(size_t i = 0; i < info.num_irqs; i++) {
                ...
                if(ret < 0)
                        throw RuntimeError(
                            "Failed to get IRQ {} of VFIO device: {}",
                            i,
                            name);
...
void *Device::regionMap(size_t index)
{
        ...
        log->debug("Mapping region {} of size 0x{:x} with flags 0x{:x}",
                   index,
                   r->size,
                   flags);
        mappings[index] = mmap(nullptr,
                               r->size,
...
bool Device::pciMsiFind(int nos[])
{
        ...
        while(fgets(line, sizeof(line), f)) {
                ...
                if(ret == 2) {
                        if(strstr(this->name.c_str(), name)
                           == this->name.c_str())
                                nos[idx] = irq;
                }
        }
