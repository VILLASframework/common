...
HostRam::HostRamAllocator::allocateBlock(size_t size)
{
        ...
        const void *addr
            = mmap(nullptr, size, mmap_protection, mmap_flags, 0, 0);
        if(addr == nullptr) {
                throw std::bad_alloc();
        }
        ...
        auto blockAddrSpaceId
            = mm.getProcessAddressSpaceMemoryBlock(name.str());

        const auto localAddr = reinterpret_cast<uintptr_t>(addr);
        std::unique_ptr<MemoryBlock, MemoryBlock::deallocator_fn> mem(
            new MemoryBlock(localAddr, size, blockAddrSpaceId),
            this->free);
      ...
      allocationCount(0)
{
        ...
        free = [&](MemoryBlock *mem) {
                logger->debug(
                    "Deallocating memory block at local addr {:#x} (addr "
                    "space {})",
                    mem->getOffset(),
                    mem->getAddrSpaceId());
                ...
                if(allocationCount == 0) {
                        logger->debug(
                            "All allocations are deallocated now, freeing "
                            "memory");
                ...
                logger->debug("Available memory: {:#x} bytes",
                              getAvailableMemory());
        };
}
...
LinearAllocator::allocateBlock(size_t size)
{
        ...
        auto addrSpaceName
            = mm.getSlaveAddrSpaceName(getName(), blockName.str());
        ...
        std::unique_ptr<MemoryBlock, MemoryBlock::deallocator_fn> mem(
            new MemoryBlock(localAddr, size, addrSpaceId),
            this->free);
...
{
        free = [&](MemoryBlock *mem) {
                if(::munmap(reinterpret_cast<void *>(mem->getOffset()),
                            mem->getSize())
                   != 0) {
      ...
      num(num)
{
        ...
        if(getSize() == 0) {
                logger->error(
                    "Zero-sized DMA buffer not supported, is the kernel "
                    "module loaded?");
                throw std::bad_alloc();
        }
        ...
        mm.createMapping(base,
                         0,
                         getSize(),
                         getName() + "-PCI",
                         mm.getPciAddressSpace(),
                         getAddrSpaceId());
        ...
        if(bufFd != -1) {
                ...
                if(buf != MAP_FAILED)
                        mm.createMapping(reinterpret_cast<uintptr_t>(buf),
                                         0,
                                         getSize(),
                                         getName() + "-VA",
...
HostDmaRam::HostDmaRamAllocator::~HostDmaRamAllocator()
{
        ...
        try {
                auto translation
                    = mm.getTranslationFromProcess(getAddrSpaceId());
                baseVirt
                    = reinterpret_cast<void *>(translation.getLocalAddr(0));
